...
class MyModelCheckpoint(ModelCheckpoint):
    ...
    def _save_model(self, filepath, save_weights_only=False):
        # make paths
        os.makedirs(os.path.dirname(filepath), exist_ok=True)

        # delegate the saving to the model
        if self.save_function is not None:
            weights_only = (self.save_weights_only or save_weights_only)
    ...
    def format_checkpoint_name(self, epoch, metrics, step, ver=None):
        ...
        if len(groups) == 0:
            ...
            for tmp in groups:
                name = tmp[1:]
                filename = filename.replace(tmp, name + '_{' + name)
    ...
    def on_validation_end(self, trainer, pl_module):
        ...
        while os.path.isfile(filepath):
            filepath = self.format_checkpoint_name(epoch, metrics, step, ver=version_cnt)
    ...
    def _do_check_save(self, filepath, current, epoch):
        ...
        self._save_model(filepath, save_weights_only=True)
